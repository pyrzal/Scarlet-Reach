	...
	switch(riteselection) // put ur rite selection here
		...
		if("Mark of Baotha")
			...
			if(!target || QDELETED(target) || target.loc != loc)
				return
			if(do_after(user, 50))
				user.say("#Lady of Lust, hear my prayer...")
				if(do_after(user, 50))
					user.say("#I offer to you this mortal vessel...")
					if(do_after(user, 50))
						user.say("#Accept my offering, and bless their flesh with thy unholy boon!")
						if(do_after(user, 50))
							user.say("Praise Baotha!")
							icon_state = "baotha_active" 
...
/obj/structure/ritualcircle/baotha/proc/baothaconversion(mob/living/carbon/human/target)
	...
	if(prompt == "RESIST")
		...
		target.Knockdown(60)
		spawn(10)
			to_chat(target, span_userdanger("Your flesh is seared with unholy flames!"))
			target.emote("Agony")
